...
func request_FaqService_GetFaq_0(ctx context.Context, marshaler runtime.Marshaler, client FaqServiceClient, req *http.Request, pathParams map[string]string) (proto.Message, runtime.ServerMetadata, error) {
	...
	val, ok = pathParams["faq_id"]
	if !ok {
		return nil, metadata, status.Errorf(codes.InvalidArgument, "missing parameter %s", "faq_id")
	}

	protoReq.FaqId, err = runtime.String(val)

	if err != nil {
		return nil, metadata, status.Errorf(codes.InvalidArgument, "type mismatch, parameter: %s, error: %v", "faq_id", err)
	}
...
func RegisterFaqServiceHandlerClient(ctx context.Context, mux *runtime.ServeMux, client FaqServiceClient) error {
	...
	mux.Handle("GET", pattern_FaqService_GetFaq_0, func(w http.ResponseWriter, req *http.Request, pathParams map[string]string) {
		...
		resp, md, err := request_FaqService_GetFaq_0(rctx, inboundMarshaler, client, req, pathParams)
...
var (
	pattern_FaqService_GetFaq_0 = runtime.MustPattern(runtime.NewPattern(1, []int{2, 0, 2, 1, 1, 0, 4, 1, 5, 2}, []string{"v1alpha1", "faq", "faq_id"}, ""))
)
